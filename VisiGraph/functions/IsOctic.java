...
import edu.belmont.mth.visigraph.models.functions.*;
	...
	public String evaluate( Graphics2D g2D, Graph g, Component owner )
	{
		...
		for( Vertex vertex : g.vertices )
			if( g.getEdges( vertex ).size( ) != 8 || g.getNeighbors( vertex ).size( ) != 8 || ( g.areDirectedEdgesAllowed && g.getEdgesFrom( vertex ).size( ) != g.getEdgesTo( vertex ) ) )
				return Boolean.FALSE.toString( );
	...
	public Object getAttribute( Function.Attribute attribute )
	{
		switch( attribute )
		{
			...
			case Function.Attribute.DESCRIPTION:
				return "Returns a Boolean value indicating whether or not a given graph is octic.  A graph is said to be <i>octic</i> iff it is 8-regular.";
			case Function.Attribute.OUTPUT:
				return "<code>True</code> if the graph is octic, <code>false</code> otherwise.";
