		...
		while ( true )
		{
			...
			switch ( tokenizer.nextNonWhitespaceChar( ) )
			{
				...
				case ',':
					...
					break;
				case '}': return members;
				default: throw new Error( "Expected ',' or '}'" );
			}
		}
	}
	...
	private static class JsonScanner
	{
		...
		public String nextChars( int n )
		{
			...
			while ( pos < n )
			{
				buffer[pos] = nextChar( );
				if ( end( ) )
					throw new Error( "Substring bounds error at " + index );
				++pos;
			}
		...
		public String nextString( char quote )
		{
			...
			while ( true )
			{
				...
				switch ( c )
				{
					case 0:
					case '\n':
					case '\r': throw new Error( "Unterminated string at " + index );
					case '\\':
						c = nextChar( );
						switch ( c )
						{
							...
							case 'f' : sb.append( '\f' ); break;
							case 'r' : sb.append( '\r' ); break;
							case 'u' : sb.append( (char) Integer.parseInt( nextChars( 4 ), 16 ) ); break;
							case '"' :
							case '\'':
							case '\\':
