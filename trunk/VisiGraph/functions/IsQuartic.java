...
import edu.belmont.mth.visigraph.models.functions.*;
	...
	public String evaluate( Graphics2D g2D, Graph g, Component owner )
	{
		...
		for( Vertex vertex : g.vertices )
			if( g.getEdges( vertex ).size( ) != 4 || g.getNeighbors( vertex ).size( ) != 4 || ( g.areDirectedEdgesAllowed && g.getEdgesFrom( vertex ).size( ) != g.getEdgesTo( vertex ) ) )
				return Boolean.FALSE.toString( );
	...
	public Object getAttribute( Function.Attribute attribute )
	{
		switch( attribute )
		{
			...
			case Function.Attribute.DESCRIPTION:
				return "Returns a Boolean value indicating whether or not a given graph is quartic.  A graph is said to be <i>quartic</i> iff it is 4-regular.";
			case Function.Attribute.OUTPUT:
				return "<code>True</code> if the graph is quartic, <code>false</code> otherwise.";
