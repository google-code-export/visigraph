...
import edu.belmont.mth.visigraph.models.functions.*;
	...
	public String evaluate( Graphics2D g2d, Graph g, Component owner )
	{
		...
		double distanceA = this.colorShortestDistanceBetween( g, selectedVertices.get( 0 ), selectedVertices.get( 1 ), 0 );
		
		if( !g.areDirectedEdgesAllowed )
			return ( distanceA == Double.POSITIVE_INFINITY ? "\u221E" : String.format( "%.5f", new Object[ ] { distanceA } ) );
		
		double distanceB = this.colorShortestDistanceBetween( g, selectedVertices.get( 1 ), selectedVertices.get( 0 ), ( distanceA == Double.POSITIVE_INFINITY ? 0 : 1 ) );
		
		return "{" + ( distanceA == Double.POSITIVE_INFINITY ? "\u221E" : String.format( "%.5f", new Object[ ] { distanceA } ) ) + ", " + ( distanceB == Double.POSITIVE_INFINITY ? "\u221E" : String.format( "%.5f", new Object[ ] { distanceB } ) ) + "}";
	}
	...
	public Object getAttribute( Function.Attribute attribute )
	{
		switch( attribute )
		{
			...
			case Function.Attribute.DESCRIPTION:
				return "Colors all vertices and edges along the shortest weighted path between two selected vertices and returns the length thereof.</p><p>Using Dijkstra's algorithm, the shortest weighted path is found in <code><i>O</i>((|<i>V</i>| + |<i>E</i>|) <i>log</i> |<i>V</i>|)</code> time.";
			case Function.Attribute.INPUT:
				return "The two selected vertices.";
			case Function.Attribute.SIDE_EFFECTS:
				return "All vertices and edges along the shortest weighted path between the two selected vertices will have their colors set to <code>0</code> or <code>1</code>.  All other vertex and edge colors will be reset to <code>uncolored</code> (i.e. <code>-1</code>).  <i>Note: for directed graphs, this function will attempt to find paths in both directions.</i>";
			case Function.Attribute.OUTPUT:
				return "The length(s) of the shortest weighted path(s) between the two vertices rounded to five decimal places.";
			...
			case Function.Attribute.RELATED_FUNCTIONS:
				return new String[ ] { "Color shortest path between (unweighted)" };
	...
	{
		return "Color shortest path between (weighted)";
	}
